...
//        history.add(new Message("user",100,"text1"));
//        history.add(new Message("user",200,"text2"));
//        history.add(new Message("user",300,"text3"));
    ...
    private void doPost(HttpExchange httpExchange) {
        try {
            //String strMsg = messageExchange.getClientMessage(httpExchange.getRequestBody());
            //Message msg = new Message(strMsg);
            Message msg = messageExchange.getClientMessage(httpExchange.getRequestBody(),history.size() + 1);
            System.out.println("Get Message from User : " + msg.getName());
    ...
    private void doDelete(HttpExchange httpExchange){
        try {
            String strId = messageExchange.getClientId(httpExchange.getRequestBody());
            ...
            System.err.println("Invalid request: " + httpExchange.getRequestBody() + " " + e.getMessage());
        }
    }

    private void doPut(HttpExchange httpExchange){
        try {
            Message msg = messageExchange.getClientEditInfo(httpExchange.getRequestBody());
            for (Message message : history) {
                if(message.getId() == msg.getId() && !message.isRemoved()) {
